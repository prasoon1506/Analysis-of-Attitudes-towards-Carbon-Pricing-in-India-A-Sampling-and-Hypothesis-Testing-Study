...
import os
st.set_page_config(page_title="Sales Forecasting Dashboard",page_icon="📈",layout="wide",initial_sidebar_state="expanded")
st.markdown("""<style>.main {padding: 0rem 1rem;}.stSelectbox, .stNumberInput {margin-bottom: 1rem;}.reportview-container {background: #f0f2f6}.sidebar .sidebar-content {background: #f9f9f9;}.st-emotion-cache-1y4p8pa {max-width: 100%;}</style>""", unsafe_allow_html=True)
class DataPreprocessor:
    ...
    def _calculate_advanced_metrics(features: pd.DataFrame, current_month_data: Optional[Dict]) -> pd.DataFrame:
        ...
        features['weighted_monthly_achievement_rate'] = np.average(features[monthly_achievement_cols], weights=weights, axis=1)
        features['weighted_ags_achievement_rate'] = np.average(features[ags_achievement_cols], weights=weights, axis=1)
...
class SalesPredictor:
    ...
    def predict(self,df: pd.DataFrame, selected_zone: str, selected_brand: str, growth_weights: Dict[str, float], method_weights: Dict[str, float], 
        ...
        predictions = self._generate_predictions(features, growth_weights, method_weights, current_month_data)
        return predictions, historical_data
    def _generate_predictions(self, features: pd.DataFrame, growth_weights: Dict[str, float],method_weights: Dict[str, float],current_month_data: Optional[Dict]) -> pd.DataFrame:
        ...
        final_prediction = (method_weights['rf'] * rf_prediction +method_weights['yoy'] * yoy_prediction +method_weights['trend'] * trend_prediction +method_weights['target'] * target_prediction)
        return pd.DataFrame({'RF_Prediction': rf_prediction,'YoY_Prediction': yoy_prediction,'Trend_Prediction': trend_prediction,'Target_Based_Prediction': target_prediction,'Final_Prediction': final_prediction})
    def _get_rf_prediction(self, features: pd.DataFrame) -> np.ndarray:
        ...
        self.rf_model_monthly.fit(features[feature_cols],features['month_target_nov'] * features['weighted_monthly_achievement_rate'])
        self.rf_model_ags.fit(features[feature_cols],features['ags_target_nov'] * features['weighted_ags_achievement_rate'])
    ...
    @staticmethod
    def _calculate_trend_prediction(features: pd.DataFrame, growth_weights: Dict[str, float], current_month_data: Optional[Dict]) -> np.ndarray:
        if current_month_data:
    ...
    @staticmethod
    def _get_target_based_prediction(features: pd.DataFrame,current_month_data: Optional[Dict]) -> pd.Series:
        if current_month_data:
            return (features['avg_monthly_sales'] * features['target_achievement_rate'] * (1 + (features['current_month_yoy_growth'] - 1) * 0.3))
        return features['avg_monthly_sales'] * features['target_achievement_rate']
    @staticmethod
    def _prepare_historical_data(df_filtered: pd.DataFrame,current_month_data: Optional[Dict]) -> pd.DataFrame:
        if current_month_data:
            return pd.DataFrame({
                'Period': ['October 2023', 'November 2023', 'October 2024',f'November 2024 (First {current_month_data["days_passed"]} days)',f'November 2023 (First {current_month_data["days_passed"]} days)'],
                'Sales': [df_filtered['Total Oct 2023'].iloc[0],df_filtered['Total Nov 2023'].iloc[0],df_filtered['Monthly Achievement(Oct)'].iloc[0],current_month_data['current_year'],current_month_data['previous_year']]})
        else:
            return pd.DataFrame({'Period': ['October 2023', 'November 2023', 'October 2024'],'Sales': [df_filtered['Total Oct 2023'].iloc[0],df_filtered['Total Nov 2023'].iloc[0],df_filtered['Monthly Achievement(Oct)'].iloc[0]]})
class Dashboard:
    def __init__(self):
    ...
    def _process_uploaded_file(self, uploaded_file):
        ...
        if df is not None:
            ...
            with col1:
                predictions, historical_data = self.predictor.predict(df, zone, brand, growth_weights, method_weights, current_month_data)
    ...
    def _get_current_month_data(self, use_current_month: bool) -> Optional[Dict]:
        if use_current_month:
            ...
            if days_passed > 0 and current_year_sales > 0 and previous_year_sales > 0:
                return {'days_passed': days_passed,'total_days': calendar.monthrange(2024, 11)[1],'current_year': current_year_sales,'previous_year': previous_year_sales}
        return None
    ...
    def _display_predictions(self, predictions: pd.DataFrame, historical_data: pd.DataFrame, current_month_data: Optional[Dict]):
        ...
        historical_data['Growth'] = historical_data.apply(lambda x: f"{((x['Sales'] / historical_data.iloc[0]['Sales'] - 1) * 100):.1f}% vs Base" if x.name > 0 else 'Base',axis=1)
        st.dataframe(historical_data.style.format({'Sales': '₹{:,.2f}'}),use_container_width=True)
    ...
    def _display_metrics_and_charts(self, predictions: pd.DataFrame, historical_data: pd.DataFrame, current_month_data: Optional[Dict]):
        ...
        col1, col2 = st.columns(2)
        with col1:
            st.metric("Average Prediction",f"₹{predictions['Final_Prediction'].mean():,.2f}",delta=f"{((predictions['Final_Prediction'].mean() / historical_data['Sales'].iloc[1] - 1) * 100):.1f}% vs Last Year")
        with col2:
            if current_month_data:
                current_performance = (current_month_data['current_year'] / current_month_data['previous_year'] - 1) * 100
                st.metric("Current Month Performance",f"₹{current_month_data['current_year']:,.2f}",f"{current_performance:.1f}% vs Last Year")
        ...
        for method, color in zip(methods, colors):
            fig.add_trace(go.Bar(name=method.replace('_', ' '),y=[predictions[method].iloc[0]],marker_color=color))
        fig.update_layout(title="Comparison of Different Prediction Methods",yaxis_title="Predicted Sales (₹)",showlegend=True,height=400,barmode='group')
    ...
    def _display_current_month_analysis(self, current_month_data: Dict, predictions: pd.DataFrame):
        ...
        with col1:
            st.metric("Days Completed",f"{current_month_data['days_passed']}/{current_month_data['total_days']}",f"{(current_month_data['days_passed']/current_month_data['total_days']*100):.1f}% Complete")
        with col2:
            ...
            st.metric("Current Daily Rate",f"₹{current_daily_rate:,.2f}",f"{((current_daily_rate/previous_daily_rate - 1) * 100):.1f}% vs Last Year")
        with col3:
            projected_full_month = (current_month_data['current_year'] / current_month_data['days_passed']) * current_month_data['total_days']
            st.metric("Projected Full Month",f"₹{projected_full_month:,.2f}",f"{((projected_full_month/predictions['Final_Prediction'].iloc[0] - 1) * 100):.1f}% vs Prediction")
