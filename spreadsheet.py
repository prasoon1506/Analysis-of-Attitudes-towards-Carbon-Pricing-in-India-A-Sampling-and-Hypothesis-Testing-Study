...
class SpreadsheetApp:
    ...
    def create_new_sheet(self):
        ...
        # Create a blank DataFrame with alphabetic column names and string type
    ...
    def edit_sheet(self):
        ...
        if st.session_state.spreadsheet_data is not None and not st.session_state.spreadsheet_data.empty:
            # Create column configuration to handle all columns as editable text
            column_config = {
                col: st.column_config.TextColumn(col, required=False) 
                for col in st.session_state.spreadsheet_data.columns
            }
            
            # Use data_editor with custom column configuration
    ...
    def data_analysis_tools(self):
        ...
        # Ensure the dataframe is numeric where possible for analysis
        try:
        ...
        if filter_type == "Equal to":
            ...
            if st.button("Apply Contains Filter"):
                filtered_df = st.session_state.spreadsheet_data[
                    st.session_state.spreadsheet_data[filter_col].str.contains(filter_value, case=False, na=False)
                ]
                st.write(filtered_df)
        
        # Basic charting (only for numeric columns)
    ...
    def advanced_formulas(self):
        ...
        # Attempt to convert columns to numeric, ignoring errors
        try:
