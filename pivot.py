...
def create_navigator_excel(uploaded_file):
    """Create a new Excel with navigation sheet and data display"""
    ...
    # Create a new sheet at the beginning named "Dashboard"
    ...
    selection_cell = 'B3'
    # Create data validation with list of sheet names
    ...
    # Create a named range for the dropdown cell to use in the setup instructions
    ...
    # Create a cell with instructions for the user
    ...
    instruction_row += 1
    instructions = [
        "1. Select a sheet name from the dropdown above.",
        "2. Press Alt+F8 to open the Macro dialog (we'll use Excel's built-in functionality).",
        "3. Type 'ViewSheet' and click 'Create'.",
        "4. Copy and paste this code (this is a one-time setup):",
        "",
        "Sub ViewSheet()",
        "   Dim ws As Worksheet",
        "   Dim selectedSheet As String",
        "   selectedSheet = Range(\"SheetSelector\").Value",
        "",
        "   ' Hide all sheets except Dashboard",
        ...
        "End Sub",
        "",
        "NOTE: This setup is needed only once per file. After setup, you can use",
        "the macro directly or add a button to call it."
    ]
...
# Create alternative solution without macros
def create_no_macro_solution(uploaded_file):
    """Create a solution that doesn't require macros but still provides navigation"""
    # Read the uploaded Excel file
    file_data = io.BytesIO(uploaded_file.getvalue())
    ...
    # Create buttons for each sheet with proper hyperlinks
    row = 5
    ...
    for i, sheet_name in enumerate(sheet_names):
        if sheet_name != "Dashboard":
            ...
            # Add direct hyperlink to the sheet - using absolute reference to avoid invalid reference
            cell.hyperlink = f"#{sheet_name}!A1"
            
            # Add preview section for each sheet
            ...
            try:
                ...
                # Get first 3 rows and first 3 columns for preview
            ...
            row += 4  # Leave space between sheet entries
    ...
    instruction_row += 1
    instructions = [
        "1. Click on any sheet name to navigate to that sheet",
        "2. To return to this Dashboard, use Excel's sheet tabs at the bottom",
        "3. To hide sheets, right-click on a sheet tab and select 'Hide'",
        "4. To unhide sheets, right-click on any visible sheet tab and select 'Unhide'"
    ]
...
def create_alternative_solution(uploaded_file):
    """Create a solution with multiple individual files approach"""
    # Read the uploaded Excel file
    file_data = io.BytesIO(uploaded_file.getvalue())
    ...
    instruction_row += 1
    instructions = [
        "1. This is an index of all sheets in the original workbook",
        "2. To view individual sheets, they must be individually unhidden in Excel",
        "3. To unhide a sheet: Right-click on any sheet tab → Select 'Unhide' → Choose sheet",
        "4. To hide a sheet: Right-click on the sheet tab → Select 'Hide'"
    ]
    ...
    # Create a BytesIO object for the workbook
    output = io.BytesIO()
...
def main():
    ...
    This tool will create a new Excel file with a navigation dashboard that allows you to:
    - See all available sheets in your workbook
    - View data previews from each sheet
    - Navigate between sheets easily
    """)
