...
def create_excel_with_dashboard(uploaded_file):
    ...
    # Create dropdown for sheet selection
    dropdown_range = f"Dashboard!$C$5"
    ...
    for i, sheet_name in enumerate(sheet_names):
        ...
        for col_idx, col_name in enumerate(columns):
            formula = f'=IF(C5="{sheet_name}",INDIRECT("\'{sheet_name}\'!{xlsxwriter.utility.xl_col_to_name(col_idx)}1"),"")'
        ...
        for i, sheet_name in enumerate(sheet_names):
            ...
            if row_idx < sheet_data[sheet_name]['row_count']:
                ...
                for col_idx in range(sheet_data[sheet_name]['col_count']):
                    ...
                    cell_address = f"{xlsxwriter.utility.xl_col_to_name(col_idx)}{row_idx + 2}"
                    formula = f'=IF(C5="{sheet_name}",INDIRECT("\'{sheet_name}\'!{cell_address}"),"")'
