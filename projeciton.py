...
def prepare_features_for_optimization(df, target_month='Oct'):
    """
    Prepare features for optimization, excluding the target month data
    """
...
def generate_predictions_for_optimization(features, df, growth_weights, target_month='Oct'):
    ...
    """
    # Prepare feature columns for RF
...
def find_optimal_weights(df, zone, brand, growth_weights):
    ...
    rf_pred, yoy_pred, trend_pred, target_pred = generate_predictions_for_optimization(
        features, df_filtered, growth_weights, target_month='Oct'
    )
    ...
    result = minimize(
        objective_function,
        initial_weights,
        args=(rf_pred, yoy_pred, trend_pred, target_pred, actual_oct),
        bounds=bounds,
...
def generate_all_optimal_weights(df, growth_weights):
    """
    Generate and cache optimal weights for all zone-brand combinations
    """
    optimal_weights = {}
    ...
    if uploaded_file is not None:
        try:
            ...
            with tab2:
                ...
                # Add checkbox for using optimal weights
