...
def create_visualization(region_data, region, brand, months):
    ...
    arrow = get_arrow(total_change)
    color = get_color(total_change)
    ax4.text(0.46, 0.9, f"October 2024: {total_oct_current:.0f}", fontsize=14, fontweight='bold', ha='center')
    ax4.text(0.47, 0.85, f"vs October 2023: {total_sep_current:.0f} ({total_change:.1f}% {arrow})", fontsize=12, color=color, ha='center')
    for i, (channel, value_current, value_last) in enumerate(channel_data1):
        ...
        y_pos = 0.75 - i*0.25
        ax4.text(0.25, y_pos, f"{channel}:", fontsize=14, fontweight='bold')
