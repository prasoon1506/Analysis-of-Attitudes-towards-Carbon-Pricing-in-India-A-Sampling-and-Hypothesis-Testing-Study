...
SALES_RE = re.compile(r'(?P<date>\d{2}\.\d{2}\.\d{4}).*?Sales of-(?P<ctype>[A-Za-z0-9 /+\-&]+?)\s+'r'(?P<qty>\d+(?:\.\d+)?)\s+'r'(?P<rate>\d{1,3}(?:,\d{3})*(?:\.\d+)?)\s+'r'(?P<debit>\d{1,3}(?:,\d{3})*(?:\.\d{2}))'r'(?:\s+(?P<credit>\d{1,3}(?:,\d{3})*(?:\.\d{2})))?')
def _to_float(s: str):
...
def parse_sales(lines):
    ...
    for ln in lines:
        ...
        dt=_dt(m.group("date")); 
        if not dt: continue
        rows.append(dict(date=dt,product=_norm_product(m.group("ctype")),qty_mt=_to_float(m.group("qty")),rate_bag=_to_float(m.group("rate")),debit=_to_float(m.group("debit")),credit=_to_float(m.group("credit")) if m.group("credit") else None))
...
def export_excel(report_df, all_qty, company_name="JKS", region="Unknown", period="Apr'24–Mar'25"):
    ...
    ws["A1"] = f"{company_name} ({region} {period})"
    ...
    ws[f"C{start+3}"] = f"{company_name}-Wonder"
    ws[f"D{start+3}"] = "Diff"
