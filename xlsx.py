...
st.markdown("""<style>.main {padding: 2rem;}.stAlert {padding: 1rem;margin: 1rem 0;}</style>""", unsafe_allow_html=True)
...
def safe_stats_calculation(data):
    ...
    try:
        ...
        skewness = 0
    return {'Median': np.median(data),'Mode': mode_value,'Mode Frequency': mode_freq,'Q1': np.percentile(data, 25),'Q3': np.percentile(data, 75),'Min': min(data),'Max': max(data),'Skewness': skewness}
def format_excel_report(df):
    ...
    for region in df['new_region1'].unique():
        ...
        for brand in brands:
            monthly_stats[brand] = {}
            for month in months:
                brand_month_data = region_data[(region_data['Brand: Name'] == brand) & (region_data['Month'] == month)]['Whole Sale Price'].values
...
def main():
    ...
    uploaded_file = st.file_uploader("Upload your Excel file (must contain columns: new_region1, Brand: Name, checkin date, Whole Sale Price)",type=['xlsx', 'xls'])
        ...
        try:
            ...
            if st.button("Generate Excel Report", type="primary"):
                with st.spinner('Generating report...'):
                    ...
                    st.download_button(label="📥 Download Excel Report",data=excel_buffer,file_name="brand_price_analysis_report.xlsx",mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
