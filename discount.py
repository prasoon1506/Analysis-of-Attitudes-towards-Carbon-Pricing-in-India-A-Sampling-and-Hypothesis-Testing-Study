...
st.markdown("""<style>/* Global Styles */[data-testid="stSidebar"] {background-color: #f8fafc;border-right: 1px solid #e2e8f0;}.stButton button {background-color: #3b82f6;color: white;border-radius: 6px;padding: 0.5rem 1rem;border: none;transition: all 0.2s;}.stButton button:hover {background-color: #2563eb;box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1);}/* Ticker Animation */@keyframes ticker {0% { transform: translateX(100%); }100% { transform: translateX(-100%); }}.ticker-container {background: linear-gradient(135deg, #1e293b 0%, #0f172a 100%);color: white;padding: 16px;overflow: hidden;white-space: nowrap;position: relative;margin-bottom: 24px;border-radius: 12px;box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1);}.ticker-content {display: inline-block;animation: ticker 2500s linear infinite;animation-delay: -1250s;padding-right: 100%;will-change: transform;}.ticker-content:hover {animation-play-state: paused;}.ticker-item {display: inline-block;margin-right: 80px;font-size: 16px;padding: 8px 16px;opacity: 1;transition: opacity 0.3s;background: rgba(255, 255, 255, 0.1);border-radius: 8px;}/* Enhanced Metrics */.state-name {color: #10B981;font-weight: 600;}.month-name {color: #60A5FA;font-weight: 600;}.discount-value {color: #FBBF24;font-weight: 600;}/* Card Styles */.metric-card {background: white;padding: 1.5rem;border-radius: 12px;box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1);transition: transform 0.2s;border: 1px solid #e2e8f0;}.metric-card:hover {transform: translateY(-2px);box-shadow: 0 10px 15px -3px rgba(0, 0, 0, 0.1);}.metric-value {font-size: 2rem;font-weight: 600;color: #1e293b;}.metric-label {color: #64748b;font-size: 0.875rem;margin-top: 0.5rem;}/* Chart Container */.chart-container {background: white;padding: 1.5rem;border-radius: 12px;box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1);margin: 1rem 0;border: 1px solid #e2e8f0;}/* Selectbox Styling */.stSelectbox {background: white;border-radius: 8px;border: 1px solid #e2e8f0;}/* Custom Header */.dashboard-header {padding: 1.5rem;background: linear-gradient(135deg, #1e293b 0%, #0f172a 100%);color: white;border-radius: 12px;margin-bottom: 2rem;text-align: center;}.dashboard-title {font-size: 2rem;font-weight: 600;margin-bottom: 0.5rem;}.dashboard-subtitle {color: #94a3b8;font-size: 1rem;}</style>""", unsafe_allow_html=True)
...
class DiscountAnalytics:
    def __init__(self):
        self.excluded_discounts = ['Sub Total','TOTAL OF DP PAYOUT','TOTAL OF STS & RD','Other (Please specify)','G. TOTAL']
                ...
                for discount in self.get_discount_types(df, state):
                    if discount != self.combined_discount_name:
                        ...
                        if len(filtered_df) > 0:
                            ...
                            discount_items.append(f"{discount}: <span class='discount-value'>₹{actual:,.2f}</span>")
            else:
                for discount in self.get_discount_types(df, state):
                    mask = df.iloc[:, 0].fillna('').astype(str).str.strip() == discount.strip()
                    filtered_df = df[mask]
    ...
    def create_monthly_metrics(self, data, selected_state, selected_discount):
        ...
        if selected_discount == self.combined_discount_name:
            monthly_data = {month: self.get_combined_data(df, cols, selected_state)for month, cols in self.month_columns.items()}
        else:
    ...
    def create_difference_chart(self, months, approved_values, actual_values, selected_state):
        ...
        for i in range(len(months)):
            color = '#10B981' if differences[i] >= 0 else '#EF4444'
    ...
    def get_combined_data(self, df, month_cols, state):
     ...
     if state_group:
        ...
        if len(filtered_df) > 0:
            ...
            combined_data['quantity'] = total_quantity / 2
...
def main():
    ...
    if uploaded_file is not None:
        ...
        st.markdown("""<div style='text-align: center; padding: 3rem; background: white; border-radius: 12px; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1);'><img src="https://via.placeholder.com/100x100" style="margin-bottom: 1rem;"><h2 style='color: #1e293b; margin-bottom: 1rem;'>Welcome to Discount Analytics</h2><p style='color: #64748b; margin-bottom: 2rem;'>Please upload an Excel file to begin your analysis.</p></div>""", unsafe_allow_html=True)
